        ...
        return (
            <Collapsible trigger="配置：CPU" open={true} transitionTime={200}>
                <table className="table table-sm table-bordered table-responsive">
                    ...
                    <tr>
                        <td>Socket(s)</td>
                        <td>{this.props.data.cpuExpected && this.props.data.registered && (
                            <EditCell ref={(me) => this.refCpuSocket = me}
                                      onEnter={() => this.updateCpuSocket()}>{this.props.data.cpuExpected.socket}</EditCell>) || "-"}</td>
                        <td>{this.props.data.cpuDetected && this.props.data.cpuDetected.socket}</td>
                    </tr>
                    <tr>
                        <td>VCPU</td>
                        <td>{this.props.data.cpuExpected && this.props.data.registered &&
                        <EditCell ref={(me) => this.refCpuVcpu = me}
                                  onEnter={() => this.updateCpuVcpu()}>{this.props.data.cpuExpected.vcpu}</EditCell> || "-"}</td>
                        <td>{this.props.data.cpuDetected && this.props.data.cpuDetected.vcpu}</td>
                    </tr>
                    <tr>
                        <td>Model</td>
                        <td>{this.props.data.cpuExpected && this.props.data.registered &&
                        <EditCell ref={(me) => this.refCpuModel = me}
                                  onEnter={() => this.updateCpuModel()}>{this.props.data.cpuExpected.model}</EditCell> || "-"}</td>
