...
class ConnectedHosts extends Component {
    ...
    handleTableChange(pagination, filter, sorter) {
        ...
        console.log(filter)
        console.log(sorter)
        this.props.fetchHosts(this.makeQueryOnTableChange(pagination, filter, sorter))
    }

    handleTagFilterChange(selectedTags) {
        console.log(selectedTags)
        this.props.fetchHosts(this.makeQueryOnTagFilterChange(selectedTags))
    }

    componentDidMount() {
        this.props.fetchHosts(this.makeQueryNow())
    }
        ...
        nextProps.items.data.list && this.setState({
            data: nextProps.items.data.list.map(host => {
                return {
                    ...
                    cpu: {editable: false, value: host.cpuDetected},
                    memory: {editable: false, value: host.memDetected},
                    disk: this.viewDiskInfo(host.diskDetected),
                    network: {editable: false, value: host.networkDetected},
                    os: host.osDetected,
    ...
    render() {
        ...
        return (
            <div>
                <Row>
                    <Col span={1}>
                        <Button onClick={() => this.props.fetchHosts(this.makeQueryNow())}><i className="fa fa-refresh"></i></Button>
                    </Col>
                    <Col span={2}>
                        <NewHostPopup/>
                    </Col>
                    <Col span={8}>
                        <Select mode="multiple" style={{ width: '100%' }} placeholder="按 Tag 过滤" onChange={(value)=>{this.props.fetchHosts(this.makeQueryOnTagFilterChange(value))}}>{this.state.tags}</Select>
                    </Col>
                </Row>
